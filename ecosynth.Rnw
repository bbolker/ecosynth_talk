...
strain_colvec <- c("#8F4847", "#52907B") ## NY99, WN02
library(deSolve)
library(reshape)
library(lattice)
...
\begin{description}
\item[People] Morgan Kain \includegraphics[width=1in]{figs/morgankain.jpeg}
...
##     guides(linetype = FALSE, fill = FALSE, size = FALSE, colour = FALSE) +
    ...
    scale_colour_manual(values=strain_colvec,
                    name = "strain") +
...
ggplot(WN02_adv,aes(fill=factor(Temperature), ## forcats::fct_rev(factor(Temperature)),
                    ...
                    x=WN02_adv)) +
  geom_violin(position=ggstance::position_dodgev(height=-1))+  ## force ordering
...
\begin{frame}
  \frametitle{Source}  
  \url{https://github.com/bbolker/ecosynth_talk}

<<qrcode, echo = FALSE>>=
library("qrcode")
plot(qr_code("https://www.math.mcmaster.ca/bolker/misc/bolker_ecosynth.pdf"))
@
\end{frame}
