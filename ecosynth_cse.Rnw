...
\author[Ben Bolker]{Ben~Bolker}
\institute{McMaster University \\
  Departments of Mathematics \& Statistics and Biology}
\date{CSE seminar, 13 November 2019}
...
<<human_cases,fig.width=5,fig.height=3.5,warning=FALSE,out.width="\\textwidth">>=
...
(ggplot(x,aes(Year,Value,linetype=Disease,colour=Type,shape=Disease))
  ...
  + scale_colour_manual(values=c("black","red"), name="", guide="none")
  + scale_shape_manual(values=c(16,21),name="")
  + ylab("annual incidence (US)")
  + annotate(geom="text",x=2015,y=5000,label="cases",size=6)
  + annotate(geom="text",x=2015,y=30,label="deaths",size=6,colour="red")
...
WN02_adv <- (R0_final_ggplot
  ...
  %>% mutate(ss=1:(nrow(R0_final_ggplot)/16)) ## hack! row_number())
  %>% select(-VL_T)
  %>% ungroup()
  %>% pivot_wider(names_from=Virus_Lineage,values_from=R0)
...
dds <- (dd
  ...
  %>% dplyr::summarise(mean=mean(Percent_Transmitting_Mean,na.rm=TRUE))
  %>% tidyr::drop_na(mean)
)
...
\begin{frame}
  \frametitle{Brought to you by \ldots}
  \begin{itemize}
  \item Hamiltonian Markov chain Monte Carlo
  \item automatic differentiation
  \item functional, flexible statistical toolboxes
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item more efficient Bayesian computation
  \item simulates particle movement on a potential surface with \emph{random} momentum
  \item \href{https://chi-feng.github.io/mcmc-demo/app.html?algorithm=HamiltonianMC&target=banana}{animation}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item ``all-star'' algorithm (cf. FFT, Metropolis-Hastings, ...)
  \item automated chain rule
  \item neural network back-propagation
  \item cost of (objective function + gradient) $\approx$ 1.5 (cost of obj); upper bound of 5 $\times$ obj \cite{griewank_automatic_1989}
  \item platforms: CppAD, Tensorflow, Stan, Theano \dots
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Commoditization of computation}
  ``a fundamental good used in commerce that is interchangeable with other commodities of the same type''

  \begin{center}
    \includegraphics[height=2.5in]{mac_pc.png}
    \end{center}

\end{frame}

\begin{frame}
  \frametitle{Statistical computation stacks}
  \begin{itemize}
  \item linear algebra (BLAS, LAPACK, Armadillo, Eigen, CHOLMOD)
  \item automatic differentiation
  \item nonlinear optimization
  \item modeling languages
    \begin{itemize}
    \item declarative (TMB, Stan)
    \item graphical (Greta, rethinking, JAGS, PyMC)
    \item formula-based
    \item GUI
    \end{itemize}
  \end{itemize}

  \textbf{Goal:} democratization of data science
